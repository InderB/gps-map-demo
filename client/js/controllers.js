...
//             //     }
//             //   }
//             //   console.log('latlng',latlng, 'location',locationPoint);
//             // })
//             var initMap = function() {
//               // console.log("initimap");
//               $scope.map = new google.maps.Map(document.getElementById('map-pathReplay'), {
...
//             initMap()
//
//           $.getJSON( "js/dataGps.json", function(resp) {
//             // Fetched datapoints in JSON format, convert lat, long and push in array
//             // for(var k=0;k<resp.length;k=k+0){
//             var k = 0;
//             while(k < resp.length){
//               console.log("in while, k",k);
//               var latlng = []
//               // for (var i = 0; i < resp[k].length; i=i+2) {
//               var i = 0;
//               while(i < resp[k].length){
//
//                 if (Date.parse(resp[k][i+1].time) == Date.parse(resp[k][i].time)) {
//                   var latStr = ''+resp[k][i].value+''
//                   var lngStr = ''+resp[k][i+1].value+''
//                   var lat =  parseFloat(latStr.slice(0,2)) + (parseFloat(latStr.slice(2,9))/60)
//                   var lng = parseFloat(lngStr.slice(0,2)) + (parseFloat(lngStr.slice(2,9))/60)
//                   latlng.push({lat:lat,lng:lng})
//                 }
//                 i = i + 2;
//                 if(i >= resp[k].length-1){
//                   console.log("K=",k,"i=",i);
//                   locationPoint.push(latlng);
//                   breakInBatches(latlng, k, function(callback, day){
//                     console.log("Callback", callback, day);
//                     if(callback){
//                       console.log(k, "path drawn");
//                       k++;
//                     }
//                   })
//                 }
...
//             }
//
//               // for (var k = 0; k<locationPoint.length; k++) {
//               //   total = Math.ceil(locationPoint[k].length/100)
//               //   var l = locationPoint[k].length
//               //   for (var i = 0; i < locationPoint[k].length; i = i + 100) {
//               //     var j = i
//               //     var max = j + 100
//               //     if (max < locationPoint[k].length) {
//               //       console.log('if j = ', j, ' max = ', max, "k =",k)
//               //       var arr = locationPoint[k].slice(j, max)
//               //       snap(arr,k,max, l,function(callback){})
//               //     } else {
//               //       max = locationPoint[k].length
//               //       console.log('else j = ', j, ' max = ', max, "k =",k)
//               //       var arr = locationPoint[k].slice(j, max)
//               //       snap(arr,k,max,l, function(callback){})
//               //     }
//               //   }
...
//         var total;
//         var count;
//         function breakInBatches(array, k, callback){
//           var lengthOfArray = array.length;
//           total = Math.ceil(lengthOfArray/100);              // no of batches
//           count = 0;
//           console.log("in breakInBatches",k,lengthOfArray, count, total, lengthOfArray);
//           // Divide array in batch of 100 points
//           for(var i = 0; i < lengthOfArray; i = i + 100){
//             var j = i;  //lower limit
//             var max = j + 100;  // upper limit
//             if(max < lengthOfArray){
//               // For each batch of 100 points
//               var batch = array.slice(j, max);  // slice out 100 points
//               console.log("Batch size:",batch.length, "j=",j, "max=",max);
//               snap(batch, k, max, lengthOfArray,function(cb1){})
//             }
//             else {
//               // for the remaining batch of less than 100 points
//               max = array.length                // change max to the last point
...
//         }
//
//       var snap = function (arr,k,max,l, cb1) {
//         //local variable pathValues
//         var pathValues = []
//         var i = 0
//         while (i < arr.length) {
//           if (arr[i]) {
//             var values = arr[i].lat + ',' + arr[i].lng
//             pathValues.push(values)
//           }
//           i++
//         }
//         if (i == arr.length) {
//           console.log('data after else', k, max, pathValues)
//           cb1(max);
//           snapToRoad(pathValues,k,max,l, function(k, cb2){
//             console.log("callback of snapToRoad",k,cb2);
//           //   return cb1(max);
//           })
//         }
//       }
//       var snapToRoad = function (pathValues,k,max,l, cb2) {
//         $.get('https://roads.googleapis.com/v1/snapToRoads', {
//           interpolate: true,
//           key: 'AIzaSyD9wVtupL4tqSA_K5Sg7knttR0FSM7pHsw',
//           path: pathValues.join('|')
//         }, function (data) {
//           console.log('data',k, max,data, total);
//           $scope.processSnapToRoadResponse(data,k,max,l, function(cb3){
//             return cb2(k,max);
//           })
//         })
//       }
...
//        var snappedCoordinates
//        $scope.processSnapToRoadResponse = function (data,k,max,l, cb3) {
//        //  var snappedCoordinates = []
//        // console.log('data.snapped',data);
//         if (count == 0) {
//           snappedCoordinates = []
//          }
...
//            snappedCoordinates.push(latlngs)
//            // snappedCoordinates = _.reverse(snappedCoordinates)
//            placeIdArray.push(data.snappedPoints[i].placeId)
//          }
//          // console.log('print data to filter',data);
//        //   var points = data.tracepoints
//        //
//        // //  var polylines = []
//        // // console.log('check',points);
//        // //var points = data.tracepoints
//        // console.log('k =',k," max=",max," l=",l)
//        //  if (!$scope.loc && k == 0 && l == l){
//        //     console.log('in max==l',points)
//        //     var flag = false
//        //     for(var i = points.length-1; i>=0;i--){
//        //       if(!(points[i]==null) && !flag){
//        //         $scope.loc = points[i]
//        //               console.log('loc 1',$scope.loc)
//        //               $scope.map.setCenter(new google.maps.LatLng($scope.loc.location[1], $scope.loc.location[0]))
//        //               var marker = new google.maps.Marker({
//        //                   position: new google.maps.LatLng($scope.loc.location[1], $scope.loc.location[0]),
//        //                   animation: google.maps.Animation.DROP,
//        //                // icon: '../assets-dash/img/testing.gif',
//        //                // optimized: false
//        //               })
//        //               console.log('inside loc',$scope.loc);
...
//          // for (var i in points) {
//          //   if (!(points[i]==null)) {
//          //    //  console.log('data before process'+points[i].location[1]+'longitude'+ points[i].location[0]);
//          //       var latlon = new  google.maps.LatLng(points[i].location[1],points[i].location[0])
//          //
//          //      // console.log('latlon ',latlon);
//          //       snappedCoordinates.push(latlon);
//          //   }
//          // }
//
//          count++
...
//
//
//            snappedCoordinates.push(snappedCoordinates[snappedCoordinates.length-1]);
//            console.log('snappedCoordinates ', snappedCoordinates.length)
//            for(var i=0; i< snappedCoordinates.length-1; i++){
//              var arr = [];
//              arr.push(snappedCoordinates[i])
//              arr.push(snappedCoordinates[i+1])
//              // arr.push(snappedCoordinates[i+2])
//              $scope.drawSnappedPolyline(i,arr);
//              count = 0
//              snappedCoordinates = []
//              polylines = []
//              return cb3();
...
//        $scope.drawSnappedPolyline = function (i, snappedCoordinates) {
//          console.log('in drawSnappedPolyline',i,'0', snappedCoordinates[0].lat(),snappedCoordinates[0].lng(),'1',snappedCoordinates[1].lat(),snappedCoordinates[1].lng())
//          // var jobj = snappedCoordinates.matchings[0].geometry
//          // var points = jobj.coordinates
//          // var coordinates = []
//          // var polylines = []
//          // console.log('check',points);
//          //
//          // for (var i in points) {
//          //   if (!(points[i]==null)) {
//          // var latlon = new  google.maps.LatLng(points[i][1],points[i][0])
//          // console.log('latlon aaya',latlon);
...
//       //   var pointsToSnap = []
//       //   if (latlng.length/100 > 0) {
//       //     var num = parseInt(latlng.length/100)
//       //     console.log("num",num);
//       //     if (latlng.length % 100 > 0) {
//       //       console.log('%',latlng.length % 100);
//       //       var deci = latlng.length % 100
//       //       for (var i = 0; i < deci; i++) {
//       //         // runSnapToRoad(latlng,locationPoint)
//       //         pointsToSnap.push(latlng[i])
//       //         count++
//       //         if (count == deci) {
//       //           runSnapToRoad(pointsToSnap,locationPoint,latlng.length,count, function(callback){
//       //
//       //             for (var j = 0; j < num; j++) {
//       //               pointsToSnap = []
//       //               console.log('count',count, 'j',j);
//       //               for (var k = count, h=0; k < 100+count; k++) {
//       //                 pointsToSnap.push(latlng[k])
//       //                 // console.log(pointsToSnap.length);
//       //                 h++;
//       //                 if (pointsToSnap.length == 100) {
//       //                   console.log('counter',h, 'k',k);
//       //                   count=count+100;
//       //                   runSnapToRoad(pointsToSnap,locationPoint,latlng.length,count, function(callback1){
//       //                     console.log("Done!");
//       //                   })
//       //                 }
//       //               }
...
//       //         }
//       //       }
//       //       console.log('pointsToSnap',pointsToSnap);
//       //     }
//       //   }
//       //   // runSnapToRoad(latlng,locationPoint)
//       //
//       //   // console.log('called',latlng[5]);
//       //   //
//       //   // var marker = new google.maps.Marker({
//       //   //    position: locationPoint[5],
//       //   //    map: $scope.map,
//       //   //    title: 'Hello World!'
//       //   //  });
//       //   // var flightPlanCoordinates = locationPoint
//       //   // // [
...
//       //   // flightPath.setMap($scope.map);
//       //   // $scope.map.setZoom(5)
//       //   // $scope.map.setCenter(locationPoint[5])
//       // }
//       function runSnapToRoad(path,rawData,orignalLength,count, cb1) {
//         console.log('orignalLength',orignalLength, 'count',count);
//         $.get('https://roads.googleapis.com/v1/snapToRoads', {
//           interpolate: true,
//           key: 'AIzaSyD9wVtupL4tqSA_K5Sg7knttR0FSM7pHsw',
//           path: path.join('|')
//         }, function(data) {
//           console.log('dataSnapped',data);
//           if (orignalLength == count) {
//             processSnapToRoadResponse(data, function(cb){
//               console.log("Processed ",data.snappedPoints.length," points!",snappedCoordinates.length);
//               console.log("Drawing Snapped Polyline:-", orignalLength, count);
//               drawSnappedPolyline(rawData,snappedCoordinates);
//             });
//           }else {
//             processSnapToRoadResponse(data, function(cb){
//               console.log("Processed ",data.snappedPoints.length," points!",snappedCoordinates.length);
...
//       snappedCoordinates = [];
//       placeIdArray = [];
//       // function processSnapToRoadResponse(data, cb) {
...
//       //     placeIdArray.push(data.snappedPoints[i].placeId);
//       //     if(i == data.snappedPoints.length-1){
//       //       console.log("snappedCoordinates",snappedCoordinates.length);
//       //       cb(true);
//       //     }
//       //   }
...
//
//       // Draws the snapped polyline (after processing snap-to-road response).
//       // function drawSnappedPolyline(rawData,snappedCoordinates) {
//       //   console.log('raw',rawData,snappedCoordinates.length);
//       //   $scope.map.setZoom(11)
//       //   $scope.map.setCenter({lat:rawData[5].lat,lng:rawData[5].lng})
